          ...
          <div key={i} id={`accion-${a.prioridad}`} className="glow-card overflow-hidden rounded-2xl">
            ...
            >
              <div className="flex items-center gap-3">
                <span className="flex h-7 w-7 items-center justify-center rounded-full border border-ribuzz-accent/25 bg-ribuzz-accent/[0.06] text-xs font-bold text-ribuzz-pink shadow-[0_0_14px_rgba(230,37,255,0.1)]">
                  {a.prioridad}
                </span>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase tracking-wide text-ribuzz-pink">
        {label}
      </p>
